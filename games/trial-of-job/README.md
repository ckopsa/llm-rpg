...
Node stdio client); the Act I day-loop rebuild (llm-rpg-565.3) was applied by a
scripted edit instead, and the golden paths below are regenerated by driving
the `Sim` directly rather than hand-counted. `sprites.json` is
hand-written; the Forge has no sprites tool yet (llm-rpg-85g.9).

**Act I is a day loop.** Job 1:1–5 describes a *routine* — a great household,
ten children feasting house by house, and a man who rises early and offers
"continually". So Act I plays as three days in Uz with **two tendings a day**:
you walk the estate and put your hands on two of the eight things in it (the
sheep, the camels, the oxen, the boy on the hill, Zabad the plowman, your wife,
your eldest son, your daughter), the light goes, and dawn finds you at the
altar. Six tendings across eight things — you cannot reach everyone, and each
one you return to opens further than the last.

Then Job 1:13–19 takes all of it, and **the messengers read your ledger back**:
the plowman who survives is either "Zabad, who told you he would be in the west
field on Thursday" or "a plowman you never asked anything". The scripture is
untouched and the outcome is fixed; what the day loop buys is that the loss
lands on things the player spent turns on rather than on a paragraph.
...
|---|---|---|---|
| I — The Land of Uz | `uz-estate` (16×12), `feast-house` (9×7) | altar, wife, shepherd boy, plowman, sheep, camels, oxen, donkeys, herd dog, courtyard dust; eldest son, daughter, feast table | `act-one-open` (enter, cold open), `the-day-turns` (**turn**, dusk + the dog's patrol), `the-day-turns-indoors` (**turn**, dusk in the feast house), `court-of-heaven` (step, after the third offering) |
...
10 maps · 36 placed entities · 14 triggers · 6 entities with variants ·
**63 cited scripture passages** and 67 uncited narration passages · 3 endings.

Cutscene vocabulary used: `spawn_entity`, `move_entity` (including `quiet` for
the dog's ambient patrol), `set_tile`,
...
| | |
|---|---|
| Budget | `spent` — two tendings a day, tracked in a var so the observer shows it |
| Day | `day` — 1, 2, 3; bumped by each offering |
| Ledger | `tended_<entity>_<n>` flags, so depth stays out of the `Vars:` line |
| Clock | a `turn` trigger on each Act I map: at `spent == 2` it narrates dusk, fades, and teleports you to the altar for the morning |
| Ambient | the same trigger walks `herd-dog` a four-beat square with `quiet: true` — cue for the renderer, no event spam |

The altar refuses an early offering ("Job offered when the days of their
feasting had run their course — and this day has not run"), which is what makes
the budget legible without a HUD. Dusk teleports you home rather than making
you walk back, so a day is: spend two acts wherever you like, then one interact
at the stone.

Ambient motion deliberately does **not** move a tendable entity — a thing that
wanders off while you walk toward it reads as the world fighting you, not
living. The dog patrols the southwest grass, out of the traffic between the
altar, the road and the feast house.
...
|---|---|---|
| `victory` | candor ≥ 4 → "I lay my hand on my mouth" → pray for the three friends → the gate | golden path below, 152 turns |
...
```
# Day 1 — the boy on the hill, then the camels; dusk; dawn at the altar
# Day 2 — Zabad the plowman, then your wife; dusk; dawn at the altar
# Day 3 — through the portal to the feast house: your eldest son, your
#         daughter; you walk home in the dark; the third offering
# then the court of heaven, the road east, and the four messengers
```

One line, as the CLI wants it (regenerated by driving the Sim directly, so it
is verified rather than hand-counted):

```
north,east,north,north,north,north,west,interact,east,east,east,east,east,interact,interact,east,north,north,east,east,east,east,east,east,east,east,interact,west,south,interact,interact,east,north,north,east,east,east,east,east,north,north,west,west,interact,east,east,east,east,interact,interact,west,south,south,east,east,east,east,interact,interact,interact,north,north,interact,choose1,east,east,east,east,east,north,interact,interact,interact,interact,interact,interact,interact,north,east,east,interact,choose1,south,east,east,interact,choose1,west,south,west,interact,choose1,north,north,interact,choose1,south,east,east,interact,choose1,west,south,west,interact,choose1,east,south,south,east,east,interact,north,north,interact,north,north,interact,north,north,interact,north,north,interact,north,interact,choose1,north,north,north,north,east,interact,east,east,east,interact,east,east,east,interact,south,south,south,west,west,east,east,east,east,east,interact
```

### `the-story-untold` (64 turns)

```
north,east,north,north,north,north,west,interact,east,east,east,east,east,interact,interact,east,north,north,east,east,east,east,east,east,east,east,interact,west,south,interact,interact,east,north,north,east,east,east,east,east,north,north,west,west,interact,east,east,east,east,interact,interact,west,south,south,east,east,east,east,interact,interact,interact,north,north,interact,choose2
```
...
```
north,east,north,north,north,north,west,interact,east,east,east,east,east,interact,interact,east,north,north,east,east,east,east,east,east,east,east,interact,west,south,interact,interact,east,north,north,east,east,east,east,east,north,north,west,west,interact,east,east,east,east,interact,interact,west,south,south,east,east,east,east,interact,interact,interact,north,north,interact,choose1,east,east,east,east,east,north,interact,interact,interact,interact,interact,interact,interact,north,east,east,interact,choose2,south,east,east,interact,choose2,west,south,west,interact,choose2,north,north,interact,choose2,south,east,east,interact,choose2,west,south,west,interact,choose2,east,south,south,east,east,interact,north,north,interact,north,north,interact,north,north,interact,north,north,interact,north,interact
```

## Validation and playtest status

- `npm run validate -- games/trial-of-job/game.json` — **valid, 0 errors, 0 warnings**.
- Explore autoplayer (maxSteps 4000, seed 1) — **win, ending `victory`,
  250 steps**, 22 of 36 entities interacted with, only `heaven-court`
  unvisited. Coverage is *lower on purpose now*: the Act I budget means an
  explorer that spends its days on the sheep and the camels never gets to the
  plowman, which is the mechanic working.
- Script mode (golden path above) — **win, 152 steps, 0 rejected**.
- All three endings re-verified against the rebuilt Act I.
...
| | before the day loop | now |
|---|---|---|
| golden path | 107 turns | 152 |
| moves / interacts / choices | 69 / 30 / 8 | 108 / 36 / 8 |
| decisions in Act I | 0 | 6 tendings + 3 offerings |

Movement is still ~70% of inputs, and that is the next thing worth attacking —
but it is now the execution of a route the player chose rather than walking
between fixed beats. See **llm-rpg-565.5** and **llm-rpg-565.6** for the same
treatment applied to the ash heap and the whirlwind.
...
- **Candor is only half-invisible.** The design calls for no meter; the text
  ...
  is no way to mark a var hidden. See **llm-rpg-wc2**. The day loop adds `day`
  and `spent` (wanted — that pair *is* the day's HUD) and `pulse`, the dog's
  ambient phase counter, which is pure noise and wants the same fix.
- **Interact is not facing-based.** With a two-act budget, an `interact` that
  resolves to the wrong adjacent entity now costs a real day, not just a turn.
  The Act I entities are placed so that every tendable has at least one
  unambiguous stand tile, but the underlying bug is **llm-rpg-3pj** and it is
  more expensive than its P2 suggests.
