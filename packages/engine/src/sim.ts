import type { Command, Entity, Game, InventoryEntry, MapDef, Portal } from "./schema.js";
...
export class Sim {
  ...
  private doMove(dir: Direction, events: string[]): void {
    ...
    if (portal) {
      ...
      if (tile.wild && zone && this.rng.chance(zone.rate)) {
  ...
  private doBattleAction(
    action: Exclude<Action, { type: "move" } | { type: "interact" }>,
    events: string[],
  ): void {
  ...
  private doInteract(events: string[]): void {
    ...
    // An undefeated trainer battles instead of chatting.
    if (adjacent.trainer && !this.state.flags.includes(adjacent.trainer.defeatFlag)) {
      this.startTrainerBattle(adjacent, events);
      return;
    }
    const interaction = adjacent.interactions.find(
      (i) =>
    ...
    for (const cmd of interaction.commands) {
      this.runCommand(adjacent, cmd, events);
    }
  }

  private runCommand(source: Entity, cmd: Command, events: string[]): void {
    switch (cmd.type) {
      case "say":
 ...
 * Battle: "move1".."move4" (or "m1".."m4"), "switch1".."switch6" (or
 * "s1".."s6" — bare "s" still means south), "item1".."item9", "catch"
 * (alias "snare"), "run".
 */
